...
class Inspector {
  ...
  inspectCPU() {
    ...
    let command = this.runCommand(
      "grep 'model name\t:' /proc/cpuinfo | head -1"
    );
    let CPUModelName = command.replace("model name\t:", "").trim();
    this.attributes["cpuType"] = CPUModelName;

    command = this.runCommand("grep 'vendor_id\t:' /proc/cpuinfo | head -1");
    let CPUVendor = command.replace("vendor_id\t: ", "").trim();
    this.attributes["cpuVendor"] = CPUVendor;

    let out = this.runCommand("grep -m1 '^cpu MHz' /proc/cpuinfo || true");
    // e.g. "cpu MHz        : 2499.998\n"
    let mhzStr = out.replace(/cpu MHz\s*:\s*/i, "").trim();
    this.attributes.cpuFrequencyMHz = mhzStr ? parseFloat(mhzStr) : null;

    command = this.runCommand("grep 'flags\t\t:' /proc/cpuinfo | head -1");
    let CPUFlags = command.replace("flags\t\t: ", "").trim();
    this.attributes["cpuFlags"] = CPUFlags.split(" ");

    command = this.runCommand("grep 'cache size\t:' /proc/cpuinfo | head -1");
    let CPUCacheSize = command.replace("cache size\t: ", "").trim();
    this.attributes["cpuCacheSizeKB"] = CPUCacheSize;
  ...
  inspectCPUDelta() {
    if (this.inspectedCPU) {
      ...
      for (let i = 0; i < metricsNames.length; i++) {
        this.attributes[metricsNames[i] + "Delta"] =
          parseInt(CPUMetrics[i]) - this.attributes[metricsNames[i]];
      }
      ...
      contextSwitches = contextSwitches.replace("ctxt ", "");
      this.attributes["contextSwitchesDelta"] =
        parseInt(contextSwitches) - this.attributes["contextSwitches"];
    } else {
      this.attributes["SAAFCPUDeltaError"] =
        "CPU not inspected before collecting deltas!";
    }
  }
  ...
  inspectMemory() {
    ...
    this.attributes["totalMemory"] = parseInt(
      lines[0].replace("MemTotal:", "").replace(" kB", "").trim()
    );
    this.attributes["freeMemory"] = parseInt(
      lines[1].replace("MemFree:", "").replace(" kB", "").trim()
    );
  ...
  inspectMemoryDelta() {
    if (this.inspectedMemory) {
      if (fs.existsSync("/proc/vmstat")) {
        ...
        lines.forEach((line) => {
          if (line.indexOf("pgfault") != -1) {
            this.attributes["pageFaultsDelta"] =
              parseInt(line.split(" ")[1]) - this.attributes["pageFaults"];
          } else if (line.indexOf("mgmajfault") != -1) {
            this.attributes["majorPageFaultsDelta"] =
              parseInt(line.split(" ")[1]) - this.attributes["majorPageFaults"];
          }
        });
      } else {
        this.attributes["SAAFMemoryDeltaError"] =
          "/proc/vmstat does not exist!";
      }
    } else {
      this.attributes["SAAFMemoryDeltaError"] =
  ...
  inspectPlatform() {
    ...
    if (key != null) {
      ...
      this.attributes["functionMemory"] =
      ...
      if (key != null) {
        ...
        this.attributes["functionMemory"] =
          process.env.X_GOOGLE_FUNCTION_MEMORY_MB;
        this.attributes["functionRegion"] =
        ...
        if (key != null) {
          ...
          this.attributes["vmID"] = this.runCommand(
  ...
  inspectAllDeltas() {
    ...
    if ("frameworkRuntime" in this.attributes) {
      this.addTimeStamp(
        "userRuntime",
        this.startTime + this.attributes["frameworkRuntime"]
      );
    }
