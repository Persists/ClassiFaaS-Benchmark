...
function extractParameter(event, defaultValue) {
  if (event.queryStringParameters && event.queryStringParameters.parameter)
    return parseInt(event.queryStringParameters.parameter, 10);
  return defaultValue;
}



const benchmarkFactory = ({ benchmarkFn, defaultParam }) => {
  return async (event, context) => {
    ...
    inspector.addAttribute("provider", "aws");
